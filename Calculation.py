...
def build_grn_input(depth_number, calculation_settings, override):
    ...
    dir = TEMP_PREFIX + 'grn/' + str(depth_number)
    ...
    with open(TEMP_PREFIX + 'grn_input/' + str(depth_number) + '.grn', 'w') as grn_input, \
        ...
        grn_input.write(f"'{TEMP_PREFIX}grn/{depth_number}/'\n")
...
def build_cmp_input(depth_number, observation_distance, configs, override):
    ...
    dir = TEMP_PREFIX + 'cmp/' + str(depth_number)
    ...
    with open(TEMP_PREFIX + 'cmp_input/' + str(depth_number) + '.cmp', 'w') as cmp_input, \
        ...
        cmp_input.write(f"'{TEMP_PREFIX}cmp/{depth_number}/'\n")
        ...
        cmp_input.write(f"'{TEMP_PREFIX}grn/{depth_number}/'\n")
...
if __name__ == '__main__':
    ...
    if ifgrn == 'y':
        ...
        for depth_number in range(depth_range[0], depth_range[1] + 1):
            build_grn_input(depth_number, calculation_settings, overide)
    if ifcmp == 'y':
        print('INFO: cmp_input.py running...')
        overide = False
        if ifoverridecmp == 'y':
            overide = True
        build_cmp_input(overide)
